...
/**
 * 设置单元格-文本框-自动完成值，AC=Auto Complete 目前限制，输入值后结果只能有一个，多个点击后没有反应，因为不可见
 * @param view1
 * @param f
 */
function setTableCellACValue(view1, f) {
    var i = f.index;
    var value = f.value;
    var table = getTableView(view1, 0);
    var cell = getTableCell(table, i);
    var field = getTextField(cell, 0);
    tap(field);
    // 预先查询结果列表
    var table2 = getTableView(view1, 1);
    var searchBar = getSearchBar(table2, 0);
    setValueByKeyboard(searchBar, value);
    delay();
    var kb = app.keyboard();
    tap(getButton(kb, "Search"));
    /* cell 不可见，无法点击，这很可能是环境问题
    table = getTableView(view1, 0);
    cell = getTableCellByText(table, f.p1["匹配"]);
    tap(cell);
    */
}
/**
...
/**
 * 设置单元格-文本框-单选，SC=Single Choice
 * @param view1
 * @param f
 */
function setTableCellSCValue(view1, f) {
    var i = f.index;
    var table = getTableView(view1, 0);
    var cell = getTableCell(table, i);
    var field = getTextField(cell, 0);
    ...
    // delay();
    // 新的弹出的表格
    table = getTableView(view1, -1);
    // logDebug("table="+table+",text="+f.value);
    cell = getTableCellByText(table, f.value);
    tap(cell);
}
/**
 * 设置密码框值
 ...
 * @param f
 */
function setTableCellTFValue(view1, f) {
    var i = f.index;
    var value = f.value;
    var table = getTableView(view1, 0);
    var cell = getTableCell(table, i);
 ...
 * @param v
 */
function setValue(e1, v) {
    if(e1 && !isUIAElementNil(e1)) {
        try {
            e1.setValue(v);
        }catch(e) {
            logError(e1+" setValue error="+e);
        }
    }
}
/**
 ...
 * @param value
 */
function setValueByKeyboard(e1, value) {
    setValue(e1,"");
    var kb = app.keyboard();
    for (var i = 0; i < value.length; i++) {
        var c = value.charAt(i);
...
function tapMenu1(menu_name) {
    ...
    var tab = window.tabBar();
    return tapButton(tab, menu_name);
}
/**
 * 点击2级菜单
 * @param menu_name
...
function tapMenu2(menu_name) {
    ...
    var menu = window.collectionViews()[0].cells()[menu_name];
    return tap(menu);
}
/**
 * 点击3级菜单
...
function tap(obj, scroll) {
    ...
    if (obj && !isUIAElementNil(obj)) {
        ...
        msg += ",after delay 可见=" + obj.isVisible();
        if (obj.isVisible()) {
            obj.tap();
            ret = true;
        } else {
