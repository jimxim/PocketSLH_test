...
/** tablecell textfield auto complete */
var TC_AC = "tc-ac";
/** tablecell textfield */
var TC_TF = "tc-tf";
/** tablecell textfield single choice */
var TC_SC = "tc-sc";
...
function getSearchBar(view1, i) {
    var bars = view1.searchBars();
    return bars[i];
}
/**
 * 获取静态文本对象
 ...
 * @returns
 */
function getTableCell(tableView1, i) {
    var a1 = tableView1.cells();
    if (i < 0) {
        i = a1.length + i;
    }
    var cell = a1[i];
 ...
 * @returns
 */
function getTableGroup(tableView1, i) {
    var a1 = tableView1.groups();
    if (i < 0) {
        i = a1.length + i;
    }
    return a1[i];
 ...
 * @returns
 */
function getTableCellByText(tableView1, text) {
    var ret;
    var cells = tableView1.cells();
    for(var i in cells) {
        var cell = cells[i];
        var name = cell.name();
        if(isIn(name,text)) {
...
function getValue(e1) {
    var ret = "";
    if(e1 && !isUIAElementNil(e1)) {
        ret = e1.name();
    }
    return ret;
}
...
function isUIATableCell(obj) {
    return isType(obj, "[object UIATableCell]");
}
/**
 * 是否UIA输入框
 * @param obj
 ...
 * @param username
 * @param password
 * @param p1 扩展属性，{"角色":"财务员","门店":"常青店","账套":"autotest1"} 财务员需要选择门店
 * @returns
 */
function login(username, password, p1) {
    var role = p1["角色"];
    var shop = p1["门店"];
    var accountSet = p1["账套"];
    var msg = role + username + "登录";
    UIALogger.logStart(msg);

    tap(getTextField(window, 0)); // 弹出账套选择界面
    var view1 = getTableView(window, 0);
    var cells = view1.cells();
    for ( var i in cells) {
        var v = getStaticTextValue(cells[i], 0);
        if (accountSet == v) {
            break;
        }
    }
    ...
    var f0 = new TField("用户名", TF, 1, username);
    var f1 = new TField("密码", TF_S, 0, password);
    var fields = [ f0, f1 ]; // 
    ...
    delay();
    // debugElementTree(target);
    // debugArray(app.keyboard().elements(),"keyboard keys");
    // var window2 = app.windows[1];
    // debugElementTree(window2);
    // tapButton(window2.toolbar(), "完成");

    /*
     * var cond = "window.staticTexts().length>9"; waitUntil(cond);
     * 
     * if (isDefined(shop)) { tapButton(getScrollView(), shop); }
     * 
     * var texts = window.staticTexts(); var ret = isEqualsTexts1(texts,
     * "商陆花(iPad版)"); if (ret) { UIALogger.logPass(msg + "成功"); } else {
...
function logout() {
    ...
    tap(getTableCellByText(tableView, "退出"));
}
...
function setTFieldsValue(view1, fields) {
    ...
    for ( var i in fields) {
        ...
        case BTN_MC:
            ...
            break;
        case TC_AC:
            setTableCellACValue(view1, f);
            break;
        case TC_TF:
            setTableCellTFValue(view1, f);
            break;
        case TC_SC:
            setTableCellSCValue(view1, f);
            break;
        default:
...
function tapKeyboardHide() {
    var kb = app.keyboard();
    if (isUIAElementNil(kb)) {
        logDebug("tapKeyboardHide kb=" + kb + " isVisible=" + kb.isVisible());
    } else {
        if (!kb.isVisible()) {
            delay(); // 暂停让键盘可见
            logDebug("tapKeyboardHide after delay isVisible=" + kb.isVisible());
        }
        // 完成按钮在键盘对象上面，x的95%处，y的-10%处（不属于键盘）
        kb.tapWithOptions({ tapOffset : { x : 0.95, y : -0.1 } });
    }
}
...
/**
 * 点击新增，右上角白十字,在手机版是通用方式
 */
function tapAdd() {
    tapNaviButton("whitePlus");
}

/**
 * 点击导航右上角 更多，取消...
 */
function tapNaviHasMore() {
    tapNaviButton("hasMore");
}
/**
 * 点击导航栏按钮
...
function tapRefresh() {
    // 7.01新增商圈功能，刷新按钮下移
    if (ipadVer >= 7.01) {
        window.tableViews()[0].tapWithOptions({ tapOffset : { x : 0.15,
            y : 0.96 } });
    } else {
        window.tableViews()[0].tapWithOptions({ tapOffset : { x : 0.17,
            y : 0.92 } });
    }
